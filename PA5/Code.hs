...
-}
impl_size = 5
impl_tbl = Dynamic("impl", (impl_size, ["srcmac","dstmac"]))
impl_val  = Static([[("srcmac", 0),("dstmac", 0)] | x <- [1..impl_size]]) 
impl_ptrn = [("srcmac", "dstmac")] 
...
-}
    , BRTF(impl_tbl, "i", "drop")
    , DRP
    , LBL("drop")                           -- label "lbl_end"
    , HLT]                                     -- halt
